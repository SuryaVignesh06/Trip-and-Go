...
import { api, Destination, getWeather, WeatherResponse } from "../../lib/api";
...
export default function DestinationDetailsPage() {
  ...
  async function loadDestination() {
    ...
    try {
      const data = await api<Destination>(
      ...
      if (data.latitude != null && data.longitude != null) {
        ...
        setWeatherError("Location coordinates are not available.");
      }
    } catch (reason) {
      setDestination(null);
      setError(
        reason instanceof Error
          ? reason.message
          : "Destination details could not be loaded."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <Link
        href="/destinations"
        className="text-sm font-semibold text-[#396c68]"
      >
      ...
      {!loading && destination && (
        <div className="mt-6 grid gap-5 md:grid-cols-[1.25fr_0.75fr]">
          <section>
            <div className="rounded-2xl bg-[#edf5f4] p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#5d7a78]">
                About the destination
              </p>

              <p className="mt-4 text-lg leading-8 text-slate-700">
                {destination.description ||
                  "A destination worth discovering. Explore local highlights and start shaping your trip."}
              </p>
            </div>
            ...
            <div className="mt-6">
              ...
              {destination.highlights?.length ? (
                ...
                <p className="mt-3 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">
                  More local details will appear here as they become available.
                </p>
              )}
            </div>
          </section>

          <aside className="h-fit rounded-2xl bg-[#173f45] p-6 text-white">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#9dd6ca]">
                  Live weather
                </p>
            ...
            {weather ? (
              <div className="mt-6">
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="rounded-xl bg-white/10 p-3">
                    <p className="text-xs text-slate-300">Clouds</p>
                    <p className="mt-1 font-semibold">
                      {weather.clouds.all}%
                    </p>
                  </div>
                ...
                <p className="mt-6 border-t border-white/15 pt-3 text-xs text-slate-300">
                  Source: OpenWeather
                </p>
              </div>
            ) : (
              <p className="mt-8 text-sm text-slate-300">
                {weatherError || "Loading weather information..."}
              </p>
            )}
          </aside>
        </div>
